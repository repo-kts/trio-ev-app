            ...
            <div className="relative z-10 mx-auto max-w-6xl px-6 md:px-12">
                <header className="mb-8 md:mb-12 space-y-3 md:space-y-4">
                    <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-secondary/50 px-3 py-1.5 backdrop-blur-md">
                        ...
                        <span className="text-[10px] md:text-xs font-sans uppercase tracking-widest text-textPrimary/80">
                            Insights
                        </span>
                    </span>
                    <h1 className="font-heading text-3xl md:text-5xl font-bold leading-tight text-textPrimary">
                        Stories from the{' '}
                        ...
                        {items.map((p) => (
                            ...
                            >
                                ...
                                <div className="flex flex-1 flex-col gap-2 p-3 md:gap-3 md:p-5">
                                    <div className="flex flex-wrap items-center gap-1.5 text-[10px] md:gap-2 md:text-xs text-textPrimary/50">
                                        {p.category && (
                                        ...
                                        {p.publishedAt && (
                                            <span>
                                                {format(new Date(p.publishedAt), 'd MMM')}
                                            </span>
                                        )}
                                        <span className="hidden md:inline">·</span>
                                        <span className="hidden md:inline">{p.readingMinutes} min read</span>
                                    </div>
                                    <h2 className="font-heading text-sm md:text-xl font-semibold leading-snug text-textPrimary group-hover:text-accent line-clamp-3 md:line-clamp-none">
                                        {p.title}
                                    </h2>
                                    {p.excerpt && (
                                        <p className="hidden md:block line-clamp-2 text-sm text-textPrimary/60">
                                            {p.excerpt}
                                        </p>
                                    )}
                                    <span className="mt-auto inline-flex items-center gap-1 text-xs md:text-sm font-medium text-accent">
